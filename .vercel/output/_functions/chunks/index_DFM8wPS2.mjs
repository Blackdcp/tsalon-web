...
//#endregion
//#region src/pages/tokenrank/index.astro?astro&type=style&index=0&lang.css
...
var $$Index = createComponent(async ($$result, $$props, $$slots) => {
	...
	try {
		leaderboard = await (0, import_kv.getLeaderboard)(100, timeFilter);
		stats = await (0, import_kv.getGlobalStats)(leaderboard);
	} catch (e) {
	...
	const getToolEstimatedCost = (tool, amount) => {
		let model = "default";
		...
		if (typeof amount === "number") {
			let cacheRate = .5;
			if (model === "gpt-5.6-sol") cacheRate = .93;
			else if (model === "claude-3-5-sonnet") cacheRate = .8;
			else if (model === "gemini-2.5-pro") cacheRate = .1;
			const millions = amount / 1e6;
			return millions * cacheRate * p.cache + millions * (1 - cacheRate) * (.9 * p.in + .1 * p.out);
		}
		return amount.in / 1e6 * p.in + amount.out / 1e6 * p.out + amount.cache_read / 1e6 * p.cache;
	};
	const getToolNoCache = (tool, amount) => {
		if (typeof amount === "number") {
			let cacheRate = .5;
	...
	leaderboard.forEach((user) => {
		Object.entries(user.tokens).forEach(([tool, amount]) => {
			...
			if (typeof amount === "number") {
				...
				const millions = amount / 1e6;
				globalNoCacheCost += millions * (.9 * p.in + .1 * p.out);
				globalOutputTokens += amount * (1 - cacheRate) * .1;
			} else {
				globalCacheHitTokens += amount.cache_read || 0;
				(amount.in || 0) + (amount.out || 0);
				globalNoCacheCost += amount.in / 1e6 * p.in + amount.out / 1e6 * p.out;
	...
	const averageGenerationSpeed = 80;
