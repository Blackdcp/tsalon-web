...
def get_codex_tokens(home):
    ...
    # 3. Parse SQLite databases (CodexManager / state.sqlite)
    # Run always (not only as fallback) so users who use BOTH direct codex
    # sessions AND codexmanager get both data sources counted.
    if True:
        db_paths = [
