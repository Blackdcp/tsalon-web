    ...
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    if (!exports.kv)
                        return [2 /*return*/, 'mock-token-' + crypto.randomUUID()];
                    return [4 /*yield*/, exports.kv.get("user:".concat(userId, ":token"))];
                case 1:
                    ...
                    return [4 /*yield*/, exports.kv.set("user:".concat(userId, ":token"), newToken)];
                case 2:
                    _a.sent();
                    return [4 /*yield*/, exports.kv.set("token:".concat(newToken, ":userId"), userId)];
                case 3:
                    _a.sent();
...
function getUserIdByToken(token) {
    return __awaiter(this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            if (!exports.kv)
                return [2 /*return*/, 'mock-user-123'];
            return [2 /*return*/, exports.kv.get("token:".concat(token, ":userId"))];
        });
    });
}
        ...
        return __generator(this, function (_2) {
            switch (_2.label) {
                case 0:
                    ...
                    return [4 /*yield*/, exports.kv.get("user:".concat(userId, ":device:").concat(deviceId, ":data"))];
                case 1:
                    ...
                    if (oldDeviceDataStr) {
                        try {
                            ...
                            if (parsed.tokens) {
                                for (_c = 0, _d = Object.entries(parsed.tokens); _c < _d.length; _c++) {
                    ...
                    deviceTotal = Object.values(normalizedTokens).reduce(function (acc, val) { return acc + (val.total || 0); }, 0);
                    ...
                    return [4 /*yield*/, exports.kv.set("user:".concat(userId, ":device:").concat(deviceId, ":data"), JSON.stringify(deviceData))];
                case 2:
                    _2.sent();
                    now = Date.now();
                    ...
                    if (!(historyData && Object.keys(historyData).length > 0)) return [3 /*break*/, 8];
                    return [4 /*yield*/, exports.kv.keys("user:".concat(userId, ":timeseries:*"))];
                case 3:
                    keysToFilter = _2.sent();
                    toolsInHistory_1 = new Set();
                    for (_f = 0, _g = Object.values(historyData); _f < _g.length; _f++) {
                        toolsObj = _g[_f];
                        Object.keys(toolsObj).forEach(function (t) { return toolsInHistory_1.add(t); });
                    }
                    _loop_1 = function (key) {
                        var rawEvents, events, filteredEvents, pipeline_1;
                        return __generator(this, function (_3) {
                            switch (_3.label) {
                                case 0: return [4 /*yield*/, exports.kv.lrange(key, 0, -1)];
                                case 1:
                                    rawEvents = _3.sent();
                                    events = rawEvents.map(function (str) { return typeof str === 'string' ? JSON.parse(str) : str; });
                                    filteredEvents = events.filter(function (e) { return !(toolsInHistory_1.has(e.tool) && e.deviceId === deviceId); });
                                    return [4 /*yield*/, exports.kv.del(key)];
                                case 2:
                                    _3.sent();
                                    if (!(filteredEvents.length > 0)) return [3 /*break*/, 4];
                                    pipeline_1 = exports.kv.pipeline();
                                    filteredEvents.forEach(function (e) { return pipeline_1.rpush(key, JSON.stringify(e)); });
                                    pipeline_1.expire(key, 60 * 60 * 24 * 31);
                    ...
                    _2.label = 4;
                case 4:
                    if (!(_h < keysToFilter_1.length)) return [3 /*break*/, 7];
                ...
                case 8:
                    ...
                    for (_q = 0, _r = Object.entries(tokens); _q < _r.length; _q++) {
                        ...
                        if (historyData) {
                            for (_t = 0, _u = Object.values(historyData); _t < _u.length; _t++) {
                                toolsObj = _u[_t];
                                if (toolsObj[tool]) {
                                    toolHasHistory = true;
                                    break;
                        ...
                        if (toolHasHistory) {
                            for (_v = 0, _w = Object.entries(historyData); _v < _w.length; _v++) {
                                ...
                                if (toolsObj[tool] && toolsObj[tool] > 0) {
                                    ...
                                    pipe.expire("user:".concat(userId, ":timeseries:").concat(dateStr), 60 * 60 * 24 * 31);
                                }
                            }
                            hasTimeseriesEvents = true;
                        }
                        else if (delta > 0 || (val > 0 && oldVal === 0)) {
                            isFirstRun = (oldVal === 0 && val > 0) || delta > 100000000;
                            if (isFirstRun && val > 1000) {
                                ...
                                for (i = 0; i < days; i++) {
                                    ...
                                    pipe.expire("user:".concat(userId, ":timeseries:").concat(historyDateStr), 60 * 60 * 24 * 31);
                                }
                                hasTimeseriesEvents = true;
                            }
                            else {
                                ...
                                pipe.expire("user:".concat(userId, ":timeseries:").concat(todayStr), 60 * 60 * 24 * 31);
                                hasTimeseriesEvents = true;
                            }
                        }
                    }
                    if (!hasTimeseriesEvents) return [3 /*break*/, 10];
                    return [4 /*yield*/, pipe.exec()];
                case 9:
                    _2.sent();
                    _2.label = 10;
                case 10: return [4 /*yield*/, exports.kv.keys("user:".concat(userId, ":device:*:data"))];
                case 11:
                    deviceKeys = _2.sent();
                    aggregatedTokens = {};
                    if (!(deviceKeys.length > 0)) return [3 /*break*/, 13];
                    return [4 /*yield*/, exports.kv.mget(deviceKeys)];
                case 12:
                        ...
                        if (dataStr) {
                            try {
                                ...
                                if (parsed && parsed.tokens) {
                                    for (_z = 0, _0 = Object.entries(parsed.tokens); _z < _0.length; _z++) {
                                        _1 = _0[_z], t = _1[0], v = _1[1];
                                        if (t === 'total' || t === 'history')
                                            continue;
                                        if (typeof v === 'number') {
                                            if (!aggregatedTokens[t])
                                                aggregatedTokens[t] = { total: 0, in: 0, out: 0, cache_read: 0, cache_write: 0 };
                    ...
                    _2.label = 13;
                case 13:
                    finalTotal = Object.values(aggregatedTokens).reduce(function (acc, val) { return acc + val.total; }, 0);
                    ...
                    return [4 /*yield*/, exports.kv.set("user:".concat(userId, ":data"), JSON.stringify(aggregatedData))];
                case 14:
                    _2.sent();
                    return [4 /*yield*/, exports.kv.zadd('leaderboard:total', finalTotal, userId)];
                case 15:
                    _2.sent();
                    return [2 /*return*/];
            }
        ...
        return __generator(this, function (_e) {
            switch (_e.label) {
                case 0:
                    if (!exports.kv)
                        return [2 /*return*/, []];
                    if (!(time === 'all')) return [3 /*break*/, 3];
                    return [4 /*yield*/, exports.kv.zrevrange('leaderboard:total', 0, limit - 1)];
                case 1:
                    userIds_3 = _e.sent();
                    if (!userIds_3 || userIds_3.length === 0)
                        return [2 /*return*/, []];
                    keys = userIds_3.map(function (id) { return "user:".concat(id, ":data"); });
                    return [4 /*yield*/, exports.kv.mget(keys)];
                case 2:
                    results = _e.sent();
                    return [2 /*return*/, results.filter(Boolean).map(function (res) { return JSON.parse(res); })];
                case 3: return [4 /*yield*/, exports.kv.zrevrange('leaderboard:total', 0, limit > 0 ? limit - 1 : 99)];
                case 4:
                    userIds = _e.sent();
                    ...
                    baseDataKeys = userIds.map(function (id) { return "user:".concat(id, ":data"); });
                    return [4 /*yield*/, exports.kv.mget(baseDataKeys)];
                case 5:
                    baseDataResults = _e.sent();
                    userMap = {};
                    userIds.forEach(function (id, idx) {
                        if (baseDataResults[idx]) {
                            userMap[id] = JSON.parse(baseDataResults[idx]);
                        }
                    });
                    pipe = exports.kv.pipeline();
                    ...
                    for (_b = 0, userIds_2 = userIds; _b < userIds_2.length; _b++) {
                        ...
                        for (i = 0; i < targetDates.length; i++) {
                            ...
                            if (!err && events && events.length > 0) {
                                for (_d = 0, events_1 = events; _d < events_1.length; _d++) {
                                    evStr = events_1[_d];
                                    try {
                                        ev = JSON.parse(evStr);
                                        if (!tokens[ev.tool])
                                            tokens[ev.tool] = { total: 0, in: 0, out: 0, cache_read: 0, cache_write: 0 };
                                        ...
                                        if (ev.cacheReadTokens !== undefined) {
                                            ...
                                            fallbackCache = ev.tokens * 0.5;
                                            if (ev.tool === 'cursor' || ev.tool === 'codex' || ev.tool === 'codex_proxy')
                                                fallbackCache = ev.tokens * 0.93;
                                            else if (ev.tool === 'claude')
                                                fallbackCache = ev.tokens * 0.8;
                                            else if (ev.tool === 'antigravity')
                                                fallbackCache = ev.tokens * 0.1;
                        ...
                        if (userTotal > 0) {
                            ...
                            aggregatedList.push(__assign(__assign({}, baseData), { tokens: tokens }));
                        }
                    }
                    aggregatedList.sort(function (a, b) { return b.tokens.total - a.tokens.total; });
                    return [2 /*return*/, aggregatedList.slice(0, limit)];
            }
        });
    });
}
    ...
    return __awaiter(this, arguments, void 0, function (leaderboardData) {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    if (!exports.kv)
                        return [2 /*return*/, { totalUsers: 0, totalTokens: 0 }];
                    if (leaderboardData) {
                        totalUsers_1 = leaderboardData.length;
                        totalTokens_1 = leaderboardData.reduce(function (acc, user) { return acc + (user.tokens.total || 0); }, 0);
                        return [2 /*return*/, { totalUsers: totalUsers_1, totalTokens: totalTokens_1 }];
                    }
                    return [4 /*yield*/, exports.kv.zcard('leaderboard:total')];
                case 1:
                    totalUsers = _a.sent();
                    return [4 /*yield*/, exports.kv.zrange('leaderboard:total', 0, -1, 'WITHSCORES')];
                case 2:
                    allScores = _a.sent();
                    totalTokens = 0;
                    for (i = 1; i < allScores.length; i += 2) {
                        totalTokens += Number(allScores[i]) || 0;
                    }
                    return [2 /*return*/, { totalUsers: totalUsers, totalTokens: totalTokens }];
            }
        });
    });
}
