...
export function normalizeModelId(raw = '') {
  ...
  if (id === 'gpt-5.6' || id in MODEL_PRICING && !['gemini-2.5-pro', 'default'].includes(id)) {
  ...
  if (id in LEGACY_ALIASES) return { id: LEGACY_ALIASES[id], estimated: true };
  if (id in MODEL_PRICING) return { id, estimated: true };
...
export function priceUsage(model, pricingTiers = {}) {
  ...
  const prices = MODEL_PRICING[normalized.id] || MODEL_PRICING['gpt-5.6-sol'];
